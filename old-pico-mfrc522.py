...
class MFRC522:
    ...
    def _rreg(self, reg):
        ...
        val = self.spi.read(1)
        self.cs.value(1)
        print("In the _rred with val {}".format(val[0]))
        return val[0]
        ...
        if i:
            if (self._rreg(0x06) & 0x1B) == 0x00:
                ...
                if n & irq_en & 0x01:
                    ...
                    n = self._rreg(0x0A)
                    print("Reading n Size: {}".format(n))
                    lbits = self._rreg(0x0C) & 0x07
                    print("Reading lbits: {}".format(lbits))
                    if lbits != 0:
                        bits = (n - 1) * 8 + lbits
                    else:
                        bits = n * 8
                    print("backlen: {}".format(bits))
    ...
    def _crc(self, data):
        print("Doing the crc with data {}".format(data))
    ...
    def SelectTagSN(self):
        ...
        print("Anticoll 1 uid: {}".format(uid))
        ...
        if uid[0] == 0x88 :
            ...
            print("Anticoll 2 uid: {}".format(uid) + " valid_uid: {}".format(valid_uid))
            print("The status id {}".format(status))
    ...
    def auth(self, mode, addr, sect, ser):
        return self._tocard(0x0E, [mode, addr] + sect + ser)[0]
    ...
    def read(self, addr):
        ...
        data = [0x30, addr]
        data += self._crc(data)
        print("Did crc with data {}".format(data))
        (stat, recv, backLen) = self._tocard(0x0C, data)
        print("Did ToCard in read with status: {}".format(stat) + " backdata: {}".format(recv) + " backlen {}".format(backLen))
        return stat, recv
