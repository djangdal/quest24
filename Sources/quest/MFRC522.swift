...
class MFRC522 {
    ...
    func byteArrayToIntLittleEndian(byteArray: [UInt8]) -> Int {
        var result: Int = 0
    ...
    func byteArrayToIntBigEndian(byteArray: [UInt8]) -> Int {
        var result: Int = 0
    ...
    func anticoll() -> (status: Byte, uidLittleEndian: Int?, uidBigEndian: Int?) {
        write(addr: BitFramingReg, val: 0x00)
        ...
        if status == MI_OK {
            if backData.count == 5 {
                if backData.dropLast().reduce(0, ^) != backData.last {
                    ...
                    let uidLittleEndian = byteArrayToIntLittleEndian(byteArray: Array(backData.prefix(4)))
                    let uidBigEndian = byteArrayToIntBigEndian(byteArray: Array(backData.prefix(4)))
                    return (status: status, uidLittleEndian: uidLittleEndian, uidBigEndian: uidBigEndian)
                }
            } else {
                status = MI_ERR
            }
        ...
        return (status: status, uidLittleEndian: nil, uidBigEndian: nil)
    }

    func SelectTagSN() -> (status: Byte, uidLittleEndian: Int?, uidBigEndian: Int?) {
        let (status, uidLittleEndian, uidBigEndian) = anticoll()
        if status != MI_OK {
            return (status, nil, nil)
        }
        return (status, uidLittleEndian, uidBigEndian)
    }

    func anticoll() -> (status: Byte, uid: Int?) {
        write(addr: BitFramingReg, val: 0x00)
        ...
        if status == MI_OK {
            if backData.count == 5 {
                if backData.dropLast().reduce(0, ^) != backData.last {
                    ...
                    let uid = byteArrayToInt(byteArray: Array(backData.prefix(4)))
        ...
        return (status: status, uid: nil)
    }
    
    func SelectTagSN() -> (status: Byte, uid: Int?) {
        let (status, uid) = anticoll()
        if status != MI_OK {
            return (status, nil)
        }
        return (status, uid)
